...
obj@genomeMatrices[["cg_100k_score"]] <- makeWindows(obj, stepsize = 100000, type = "CG", metric = "score", threads = 20, index = "chr_cg", nmin = 2, species = "hg38")
...
obj@reductions[["irlba"]] <- runIrlba(obj, genomeMatrices = c("ch_100k_pct", "cg_100k_score"), dims = c(5, 5))
```
...
obj <- runTsne(obj, perplexity = 5, method = "euclidean", theta = 0.5, reduction = "irlba_regressed") 
```

Optional: one useful approach for cell type ID is analyzing CG hypomethylation over promoter regions of key marker genes. While still on the cluster, it can be helpful to calculate these cluster tracks. We recommend saving the intermediate matrix of summed observations in order to save a step for differentially methylated region (DMR) analysis.
...
dimFeature(obj, colorBy = log(mch_pct)) + scale_color_gradientn(colors = c("black", "turquoise", "gold", "red")) # helps distinguish neuron vs glia populations
```
...
```{r}
histograM(obj, genes = c("TBR1", "SLC17A7", "MOG"), matrix = "cg_cluster_tracks", legend = F)
