...
The output is the result of all tests, but it is not in a very user-friendly format. Next, expand and filter the resulting list according to the desired stringency. These steps are separated so one does not have to re-compute all tests if an alteration in stringency parameters is needed. The output of *testDMR* is not really meant to be used as-is.
