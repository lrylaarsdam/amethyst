...
library("BiocManager")
BiocManager::install(c("bluster", "caret", "cowplot", "devtools", "data.table", "dplyr", "FNN", "furrr", "future", "future.apply", "GenomicRanges", "ggplot2", "ggrepel", "grDevices", "gridExtra", "igraph", "IRanges", "irlba", "janitor", "jsonlite", "Matrix", "mgcv", "methods", "pheatmap", "plotly", "plyr", "purrr", "randomForest", "RANN", "rhdf5", "rtracklayer", "rlang", "Rtsne", "scales", "stats", "stringr", "tibble", "tidyr", "umap", "utils"))
...
Now determine cluster membership with *runCluster*. Either a Louvain-based or Leiden-based strategy can be used. Assign the output column name, which will be added to the metadata. **Please note that the structure for assigning results to the metadata has changed from <v1.0.5, in an effort to synchronize amethyst commands.**
...
obj@metadata$louvain_cluster_100k <- runCluster(obj, k = 10, reduction = "irlba_cg_100k_score", method = "louvain") 
# example: run leiden-based algorithm and store within the same object
obj@metadata$leiden_cluster_100k <- runCluster(obj, k = 10, reduction = "irlba_cg_100k_score", method = "leiden") 
...
```{r, fig.width=4, fig.height=3}
set.seed(111)
obj@metadata$louvain_vmr_cluster <- runCluster(obj, k = 10, reduction = "irlba_cg_vmr_pct", method = "louvain") # consider increasing k_phenograph for larger datasets
...
If there is one you like, it is helpful to store the vector as a variable like "pal" for future use. Below we are illustrating how to store the palette and recall it for use with *dimFeature*.

```{r, fig.width=4, fig.height=3}
pal <- c("#F9AB60", "#E7576E", "#630661", "#B5DCA5") # makePalette(option = 7, n = 4) 
dimFeature(obj, colorBy = louvain_vmr_cluster, labelBy = louvain_vmr_cluster, colors = pal, pointSize = .8, reduction = "umap_irlba_cg_vmr_pct")
```
...
obj@genomeMatrices[["pbmc_vmrs_aggregated"]] <- aggregateMatrix(obj, 
                                                                matrix = "pbmc_vmrs", 
                                                                groupBy = "louvain_vmr_cluster", 
...
library(ggrepel)
dimFeature(obj, colorBy = type, labelBy = type, colors = pal, pointSize = 0.8, reduction = "umap_irlba_cg_vmr_pct")
```
